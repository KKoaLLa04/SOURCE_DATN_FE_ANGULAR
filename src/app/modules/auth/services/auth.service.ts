...
export class AuthService implements OnDestroy {
  ...
  login(dataLogin, isRememberPassword: boolean, requestOptions) {
    return this.authHttpService.login(dataLogin, requestOptions).pipe(map((res: any)=> {
      ...
      localStorage.setItem('SchoolYear', JSON.stringify(res.data.schoolYear));
      localStorage.setItem('SchoolYearFirst', res?.data?.schoolYear[0]?.id);
      localStorage.setItem('SchoolYearName', res?.data?.schoolYear[0]?.name );
      localStorage.setItem('UserInfo', JSON.stringify(res?.data?.user));
      localStorage.setItem('access_type', res?.data?.user.access_type);
      ...
      localStorage.setItem('classIdTeacher', res?.data?.user?.classes[0]?.classId);
      return res;
    }));
  }
