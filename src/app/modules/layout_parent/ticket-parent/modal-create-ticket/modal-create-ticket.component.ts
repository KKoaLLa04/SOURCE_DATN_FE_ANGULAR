...
export class ModalCreateTicketComponent implements OnInit {
 @Input() dataModal: any;
  formGroup: FormGroup;
  dataForm: any
  dataFromParent: any;
    ...
    if (this.formGroup.valid) {
      ...
      if (this.dataFromParent.nameForm == 'update') {
        ...
        dataInput = {
          ...
          leave_date: this.formatTime.transform(this.nowTimestamp, "yyy-MM-dd"),
          return_date: this.formatTime.transform(this.endDate, "yyy-MM-dd"),
          note: valueForm.reason,
        };
      }else{
        dataInput = {
          studentId: localStorage.getItem("child_id"),
      ...
      this.dataFromParent.apiSubmit(dataInput).subscribe(
        (res: any) => {
          if (this.dataFromParent.nameForm == 'update') {
            ...
            this.showMessageService.success("Gửi đơn xin nghỉ thành công, vui lòng chờ xét duyệt")
          }
          this.globalStore.isLoading = false;
  ...
  validationMessages = {
    reason: [
      {
        type: "required",
        message: 'Lý do xin nghỉ bắt buộc chọn'
      },
    ],
  };
